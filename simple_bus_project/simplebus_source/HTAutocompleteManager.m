             ...
             ignoreCase:(BOOL)ignoreCase
{
    ...
    else if (textField.autocompleteType == BusRoute)
    {
        NSMutableArray *routearray = [NSMutableArray arrayWithArray:[RouteList MR_findAll]];
        NSMutableArray *finalroutearray = [[NSMutableArray alloc] init];
        for (RouteList *routeobj in routearray)
        {
            //NSLog(@"ROUTE NAME: %@", routeobj.nameZh);
            [finalroutearray addObject:routeobj.nameZh];;
        }
        ...
        dispatch_once(&colorOnceToken, ^
                      {
                          colorAutocompleteArray = finalroutearray;
                      });
